...
for SIZE in "${MODELS[@]}"; do
    ...
    # ====================================================
    if [ "$SIZE" == "14B" ]; then
        # 14B: Usiamo 8 per evitare 'KV Cache Preemption' e avere dati puliti, NON È COSÌ COMUNQUE CI SONO I WARNING
    ...
    for BENCH in "${BENCHMARKS[@]}"; do
        ...
        for RATIO in "${RATIOS[@]}"; do
            ...
            # SKIP LOGIC: Se esiste già il file dei watt, salta (utile per 3B e 7B già fatti)
            ...
            # Pausa aumentata a 10s per sicurezza sui dati Grid5000
            sleep 10

            # === DOWNLOAD METRICHE (WATT + BMC) ===
            curl -k -s -u "${G5K_USER}:${G5K_PASSWORD}" \
            "https://api.grid5000.fr/stable/sites/${SITE}/metrics?nodes=${NODE_NAME}&metrics=wattmetre_power_watt&start_time=${START_TIME}&end_time=${END_TIME}" \
            > "${EXP_DIR}/metrics_watt.json"

            curl -k -s -u "${G5K_USER}:${G5K_PASSWORD}" \
            "https://api.grid5000.fr/stable/sites/${SITE}/metrics?nodes=${NODE_NAME}&metrics=bmc_node_power_watt&start_time=${START_TIME}&end_time=${END_TIME}" \
