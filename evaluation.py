...
def infer(args, test_data, answer_extraction_fn):
    ...
    if args.use_vllm:
        # <<< FIX CRITICO: Aggiunto max_model_len=4096 anche nel ramo senza adapter >>>
        # Questo previene il crash "KV cache" sul modello 14B limitando il contesto a 4k token (sufficienti per GSM8K)
        if args.use_adapter:
            model = LLM(model=args.model_path, tokenizer=args.tokenizer_path, trust_remote_code=True, enable_lora=True, tensor_parallel_size=len(os.environ['CUDA_VISIBLE_DEVICES'].split(",")), max_model_len=4096)
        else:
            model = LLM(model=args.model_path, tokenizer=args.tokenizer_path, trust_remote_code=True, tensor_parallel_size=len(os.environ['CUDA_VISIBLE_DEVICES'].split(",")), max_model_len=4096)
        ...
        # <<< MODIFICA WARM START: Salvataggio timestamp >>>
        try:
            inference_start_timestamp = int(time())
            with open("timing_info.json", "w") as f:
                json.dump({"start_inference": inference_start_timestamp}, f)
            print(f"DEBUG: Saved start timestamp (Warm Start): {inference_start_timestamp}")
        except Exception as e:
            print(f"WARNING: Could not save timing info: {e}")
        # <<< FINE MODIFICA >>>
        ...
        # Percorso senza vLLM (Transformers) - Nessuna modifica necessaria qui per il crash vLLM
        ...
        # <<< MODIFICA WARM START (Transformers path) >>>
        try:
            inference_start_timestamp = int(time())
            with open("timing_info.json", "w") as f:
                json.dump({"start_inference": inference_start_timestamp}, f)
